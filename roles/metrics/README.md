...
* `satellite_metrics_url`: The URL of the OpenMetrics endpoint of Satellite, defaults to `https://{{ ansible_facts['fqdn'] | default('localhost') }}/metrics`. Set to an empty string (`''`) to disable fetching metrics from Satellite.
...
* `satellite_metrics_grafana_pmproxy_url`: The URL of the `pmproxy` service to be used as the Grafana datasource, defaults to `http://{{ ansible_facts['fqdn'] | default('localhost') }}:44322`.
